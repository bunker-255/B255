...
const Header: React.FC = () => {
  ...
  return (
    ...
    <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 print:hidden ${scrolled ? 'h-16 bg-bunker-950/80 backdrop-blur-md border-b border-white/5' : 'h-20 bg-transparent border-b border-transparent'}`}>
      <div className="w-full max-w-[1400px] mx-auto px-4 lg:px-6 h-full flex items-center justify-between">
        ...
        {/* Desktop Nav - Hidden on mobile/tablet, Visible on MD+ */}
        <nav className="hidden md:flex items-center md:gap-0 xl:gap-1">
          {NAV_ITEMS.map((item) => (
             item.path.startsWith('http') ? (
                <a
                  ...
                  className={`relative px-1.5 md:px-2 xl:px-4 py-2 text-[9px] md:text-[10px] xl:text-[11px] font-mono uppercase tracking-normal xl:tracking-widest transition-all text-slate-400 hover:text-neon-green whitespace-nowrap`}
                >
        ...
        <div className="hidden md:flex items-center gap-2 xl:gap-4">
          ...
          <Link 
            to="/contact"
            className="group relative px-2 md:px-3 xl:px-5 py-1.5 md:py-2 bg-transparent text-neon-green font-mono text-[9px] md:text-[10px] xl:text-[11px] font-bold uppercase tracking-normal xl:tracking-widest border border-neon-green/30 hover:bg-neon-green hover:text-bunker-950 transition-all clip-corner overflow-hidden whitespace-nowrap"
          >
        ...
        <button 
            ...
            className="hidden text-white p-2 hover:text-neon-green transition-colors relative z-[101]"
    ...
    {/* Overlay */}
    <div 
        className={`fixed inset-0 bg-bunker-950/60 backdrop-blur-sm z-[150] transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
    ></div>

    {/* Drawer Panel */}
    <div className={`fixed top-0 h-full w-[280px] sm:w-[320px] bg-bunker-950 z-[160] transform transition-transform duration-300 ease-out flex flex-col shadow-2xl md:hidden 
                ...
                {NAV_ITEMS.map((item, idx) => (
                    item.path.startsWith('http') ? (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="group flex items-center justify-between px-6 py-3 border-b border-white/5 text-xs font-mono uppercase tracking-wider transition-all hover:bg-white/5 text-slate-400"
                        >
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-bunker-900/95 backdrop-blur-md border-t border-white/10" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="h-16 flex items-center justify-around px-2">
          ...
          {/* Menu Toggle */}
          <button onClick={() => setIsOpen(true)} className="flex flex-col items-center justify-center w-12 h-12 transition-colors text-slate-400 hover:text-white relative">
            <Menu size={20} />
            <span className="text-[9px] mt-1 font-mono">Menu</span>
            {isOpen && <div className="absolute top-1 right-2 w-2 h-2 bg-neon-green rounded-full shadow-[0_0_8px_#00ffa3]"></div>}
          </button>
      </div>
    </div>
    </>
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="relative bg-bunker-950 pt-20 pb-24 lg:pb-10 overflow-hidden border-t border-white/5 print:hidden">
      {/* Decorative Grid Background */}
      <div className="absolute inset-0 bg-grid-pattern opacity-[0.03] pointer-events-none"></div>

      <div className="w-full max-w-[1400px] mx-auto px-4 lg:px-6 relative z-10">
